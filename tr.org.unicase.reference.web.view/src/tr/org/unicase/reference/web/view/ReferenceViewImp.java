...
public class ReferenceViewImp extends AbstractView<ReferenceControllerImp> {
	...
	public ReferenceViewImp(Long entityTypeId, Long ownerId, String refOwner) {
	...
	private void displayActiveButtonActions() {
		...
		if (userRole != null) {
			List<ReferenceEntity> activeActionList = getController().findActiveActionList(userRole);
			...
			if (field.getName().indexOf("ck_") >= 0) {
				...
				if (combobox != null) {
					...
					if (combobox.getContainerDataSource() != null && combobox.getContainerDataSource().size() > 0) {
						selectedParent = combobox.getSelectedValue();
					}
	...
	private Component createComponent(Object object, String name, Class<?> clazz) {
		Component component = null;
		Field field = getController().getField(name);
	...
	public void setViewToEntity() {
		...
		if (controller.getField("ck_owner").getVisible())
			entity.setCk_owner((ReferenceEntity) comboBox_Ck_owner.getSelectedValue());
		...
		if (controller.getField("byte_01").getVisible()) {
			if (textField_byte_01.getFileContent() != null)
				entity.setByte_01(textField_byte_01.getFileContent());
			entity.setByte_01_name((textField_byte_01.getFileName() != null ? textField_byte_01.getFileName().trim() : Configuration.View.EMPTY_STRING));
		}
	}
